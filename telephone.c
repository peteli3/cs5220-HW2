...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < world_size; i++) {
        if (world_rank == i) {
            ...
            MPI_Send(buf, 1, MPI_CHAR, (world_rank + 1) % world_size, 0, MPI_COMM_WORLD);
            printf("inside %d sending %s to %d\n", world_rank, buf, (world_rank + 1) % world_size);
            ...
            MPI_Recv(buf, 1, MPI_CHAR, (world_rank + 1) % world_size, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
